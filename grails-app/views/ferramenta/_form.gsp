...
<div class="fieldcontain ${hasErrors(bean: ferramentaInstance, field: 'file', 'error')} ">
    ...
    <g:field type="file" name="file" id="file" required=""
             value="${fieldValue(bean: ferramentaInstance, field: 'file')}"/>
</div>
...
<div class="fieldcontain ${hasErrors(bean: ferramentaInstance, field: 'researchLine', 'error')} ">
    ...
    <g:select id="researchLine" name="researchLine.id" from="${rgms.publication.ResearchLine.list()}" optionKey="id"
              value="${ferramentaInstance?.researchLine?.id}" class="many-to-one" noSelection="['null': '']"/>
</div>
...
<!-- #if( $contextualInformation ) -->
     <g:select name="members" from="${PublicationController.membersOrderByUsually()}" size="10" multiple="yes" optionKey="id" value="${ferramentaInstance?.members.id}"/>
<!-- #else <g:select name="members" from="${Member.list()}" size="10" multiple="yes" optionKey="id" value="${ferramentaInstance?.members.id}"/> -->
<!-- #end -->
