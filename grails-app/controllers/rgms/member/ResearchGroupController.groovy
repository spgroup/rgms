...
class ResearchGroupController {
	...
	void notifyChangeChildOfResearchGroup(researchGroup, members) {
		for(memberId in members) {
			...
			if(member.getEmail()) {
				mailService.sendMail {
				   ...
				   body "Hello ${member.name},\n\nThe Research Group ${researchGroup.name} is now child of the Research Group ${researchGroup.getChildOf().getName()}".toString()
				}
			}
		}
	}
		...
		boolean isChildOfResearchGroupChanged(researchGroupInstance, newResearchGroupChildOf) {
			if(newResearchGroupChildOf != researchGroupInstance.getChildOf())
				return true
			else
				return false
		}
