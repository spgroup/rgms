...
class ResearchProjectPageCreatePage extends FormPage{
    ...
    def fillResearchProjectDetails(projectName, description) {
    ...
    def fillResearchProject(projectName) {
        fillResearchProjectDetails(projectName, "Neste projeto pretendemso definir e validar um método para a implementação de aplicações orientadas a Aspectos Complexas web. Em particular, este método deve suportar uma abordagem progressiva para implementação orientada a objetos, de forma que aspectos de distribuição, concorrência, e persistência não sejam inicialmente considerados pelo processo de implementação, mas sejam gradualmente introduzidos, preservando os requisitos funcionais da aplicação.")
    }

    def fillResearchProjectWithBlankDescription(projectName) {
        fillResearchProjectDetails(projectName,"")
    }
